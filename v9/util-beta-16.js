...
getRandomName: () => {
  ...
  loginXray: (ctx) => {
    ...
    ctx.console.log('XrayEnabled:', xrayEnabled, typeof(xrayEnabled));
    ...
    pm.sendRequest(
      ...
      function (err, response) {
        ...
        token = response.json();
        ctx.console.log("Xray token:", token);
  ...
  export_result: (ctx, result) => {
    const pm = ctx.pm;
    const import_xray = pm.environment.get("xray_enabled");
    if (import_xray !== "true") return;
    
    ctx.console.log("Exporting result to Xray");
    const regex = /[A-Z-0-9]{2,}-\d+/g;
    ...
    ctx.console.log("test run:", test_run_key);
    ...
    ctx.console.log("Start sending getXray....");
    pm.sendRequest(
      {
